...
// O Neon exige SSL. Criamos a configuração baseada na DATABASE_URL.
const pool = new Pool({
  connectionString: process.env.DATABASE_URL, // Use a URL completa do Neon
  ssl: {
    rejectUnauthorized: false, // Obrigatório para o Neon no Render
  },
  max: 20,
  idleTimeoutMillis: 30000,
});
...
pool.on("error", (err) => {
  logger.error("[Database Pool] Erro inesperado:", err);
});
